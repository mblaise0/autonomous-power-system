...
void startGen01() {
  ...
  if ((digitalRead(POWER_AVAILABLE_GEN01)) == POWER_AVAILABLE) {
    ...
    if (reportGen01StartError == reportGen01StartWaitingInterval){
       ...
       for (int i = 0; i < 3; i++){
          mqttClient.publish("catGenTwo", "genTwoNxt", 2);    //informing node10 that gen01 is next to run( reset node10 Gen01 counter)
          vTaskDelay(500);
       }
       vTaskDelay(4000);
       for (int i = 0; i < 3; i++){
         mqttClient.publish("strtGen", "STRTGEN", 2);
...
void stopGen01() {
  ...
  if ((digitalRead(POWER_AVAILABLE_GEN01)) == POWER_UNAVAILABLE) {
    ...
    Serial.println("Success! Gen01 has ShutDown succesfully.");
     ...
     mqttClient.publish("outTopic", data2, 2); 
     for (int i = 0; i < 3; i++){
       mqttClient.publish("catGenTwo", "genTwoNxt", 2);    //informing node09(assuming it is online) that its generators are next to run
       vTaskDelay(500);
     }
     nxtGenSignal = 0;
     startGenSignal = 3;
     ...
     mqttClient.publish("g02StrtTopic", "3", true); 
     mqttClient.publish("g02StpTopic", "3", true); 
     stopGeneratorOne.disable();  
  }
}
