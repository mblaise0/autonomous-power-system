  ...
  if ((digitalRead(POWER_AVAILABLE_GEN02)) == POWER_AVAILABLE) {
    ...
    Serial.println("Success! Gen02 has Started succesfully.");
     ...
     if (reportGen02StartError == reportGen02StartWaitingInterval){
       ...
       for (int i = 0; i < 3; i++){
          mqttClient.publish("catGenOne", "genOneNxt", 2);    //informing node10 that gen01 is next to run( reset node10 Gen01 counter)
          vTaskDelay(500);
       }
       vTaskDelay(4000);
...
//         client.publish("errMon", "335gen02", 2);
       ...
       mqttClient.publish("g02StrtTopic", data1, true);
       mqttClient.publish("g02StpTopic", data1, true); 
...
void stopGen02() {
  ...
  if ((digitalRead(POWER_AVAILABLE_GEN02)) == POWER_UNAVAILABLE) {
   ...
   mqttClient.publish("g01StrtTopic", "0", true); 
   mqttClient.publish("g01StpTopic", "0", true); 
   stopGeneratorTwo.disable();  
  }
}
