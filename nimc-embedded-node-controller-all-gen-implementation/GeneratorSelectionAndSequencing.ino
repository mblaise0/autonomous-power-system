...
void startFitGenerator() {
  String printTime = (String)(timeClient.getFormattedTime());
  Serial.print("> NTP Time:");
  Serial.println(timeClient.getFormattedTime());
  String element = printTime.substring(0,2);
  int actualHourOfADay = element.toInt();
  Serial.println("The hour of the day is: "+(String)actualHourOfADay);
  /*
   * if((actualHourOfADay == 22) || (actualHourOfADay <= 6)){ at this condition the 335KVA Generator will come on at 10pm
   * once theres utility failure, and this will last till 6am in the morning for its circle to exhaust
   */
  if((actualHourOfADay == 7) || (actualHourOfADay <= 11)){               //using the 24hr clock
    startGen335kvaGenerator.enable();
    selectFitGenerator = 0; 
    stopGenRunningCount = 2;
    ...
    if (selectFitGenerator == 1){
      ...
      startGen02.enable();
      #if DEBUG
      Serial.println("*************condition to start Gen02 has been met**************");
      #endif
      selectFitGenerator = 0;           
    }
    startGeneratorFitToRun.disable();
...
void startGenerator02Routine() {
  ...
  if ((startGen02Counter == 1) || (startGen02Counter == 5)) {
      String instruction = "STRTGEN";
      instruction.toCharArray(data2, (instruction.length() + 1));
      for (int i = 0; i < 3; i++){
  ...
  if (startGen02AllowedWaitTime == 5) {
        char data3[750];
        startGen01.enable();
        selectFitGenerator = 1;
        stopGenRunningCount = 0;
...
void startGen335kvaGeneratorRoutine() {
   char data1[750];
   startKvaGeneratorCounter++;
  if (startKvaGeneratorCounter == 1){
    String instruction = "STRTGEN";
    ...
    client.publish("fitGenStrtC", "0", true);
    stopGenRunningCount = 2;
    Serial.println("Success! Mandate To start 335KVA generator has been issued.");
  }else if (startKvaGeneratorCounter == 18){
      Serial.println("Error! No response from 335KVA Generator.");
      client.publish("outTopic", "335KVA Gen Failed Start", 2);
