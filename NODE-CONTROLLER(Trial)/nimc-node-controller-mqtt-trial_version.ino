...
Task selectAnotherFitGenerator(1000, TASK_FOREVER, &reportGeneratorFailedStart);
...
void setup() {
  ...
  userScheduler.addTask(selectAnotherFitGenerator);
...
char reciever(String Value) {
   ...
   if((Value == "UTLPWA1")){
      ...
      if (utilityGoodFlagCount == utilityGoodWaitingInterval){
          ...
          startUtilityGoodProcess.enableDelayed(1000);
   ...
   engage335Contactor.enableDelayed(2000);
     ...
     */
     ...
     if (g01runningCount == 2){   
        ...
        updateGen01Counter();
        g01runningCount = 0;
        selectAnotherFitGenerator.disable();   //disable the monitor once a gen reports to be running
     }
